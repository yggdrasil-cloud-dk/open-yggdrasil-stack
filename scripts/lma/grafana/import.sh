...
for jsonfile_no_dashboard_key in $(find $dir -iname "*.json"); do
  ...
  curl -fs http://admin:$admin_pass@10.38.0.134:3000/api/dashboards/uid/$dashboard_id -o /dev/null || curl http://admin:$admin_pass@10.38.0.134:3000/api/dashboards/import -X POST -H "Content-Type: application/json" -d @$dashboard_jsonfile
